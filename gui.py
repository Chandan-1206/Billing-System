...
def create_gui(root):
    ...
    light_theme = {
        ...
        "input_bg":         "#FFFFFF",   # entry background
    ...
    dark_theme = {
            ...
            "input_bg":         "#D89BFB",   # very deep purple
 ...
 #  styled_entry(vars["product_entry_var"], 1)
    ...
    buttons = [
        ...
        themed_button("Save Bill as PDF", lambda: save_bill_as_pdf(vars, get_bill_items(), total_label, reset, deduct_var.get()), 2)
    ]
    ...
    # Checkbox: Deduct from inventory
    deduct_checkbox = tk.Checkbutton(
        root,
        text="Deduct from inventory",
        variable=deduct_var,
        font=("Arial", 11),
        bg=current_theme["theme"]["bg"],
        fg=current_theme["theme"]["fg"],
        selectcolor=current_theme["theme"]["bg"],
        activebackground=current_theme["theme"]["bg"]
    )
    deduct_checkbox.grid(row=5, column=0, pady=(0, 10))
